...
const DatePickerMonth = ({ selectedDate, focusedDate, labelledById, localization, firstDayOfWeek, min, max, dateFormatter, isDateDisabled, onDateSelect, onKeyboardNavigation, focusedDayRef, }) => {
  ...
  return (hAsync("table", { class: "duet-date__table", "aria-labelledby": labelledById },
    ...
    hAsync("tbody", null, chunk(days, 7).map(week => (hAsync("tr", { class: "duet-date__row" }, week.map(day => (hAsync("td", { class: "duet-date__cell" },
      hAsync(DatePickerDay, { day: day, today: today, focusedDay: focusedDate, isSelected: isEqual(day, selectedDate), disabled: isDateDisabled(day), inRange: inRange(day, min, max), onDaySelect: onDateSelect, dateFormatter: dateFormatter, onKeyboardNavigation: onKeyboardNavigation, focusedDayRef: focusedDayRef }))))))))));
};
